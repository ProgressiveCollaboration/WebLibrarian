...
import com.pc.app.ui.dialog.LibraryUserDialog;
...
public class GenericCRUD extends FlexLayout {
	...
	public static Logger log = LoggerFactory.getLogger(GenericCRUD.class);
	...
	public GenericCRUD(EntityAction action) {
		...
		case PUBLICATIONS:
			...
			grid.addColumn(publication -> publication.getCreatedDate().format(DateTimeFormatter.ISO_DATE))
					.setHeader("Created");
			grid.addComponentColumn(publication -> {
				Div fl = new Div();
		...
		case AUDIOVIDEO:
			...
			gridAudioVideo.addColumn(audioVideo -> audioVideo.getCreatedDate().format(DateTimeFormatter.ISO_DATE))
					.setHeader("Created");
			gridAudioVideo.addComponentColumn(audioVideo -> {
				Div fl = new Div();
				fl.getStyle().set("textAlign", "right");
		...
		case LIBRARY_USERS:
			...
			gridUsers.addColumn(libraryUser -> libraryUser.getCreatedDate().format(DateTimeFormatter.ISO_DATE))
					.setHeader("Created");
			gridUsers.addComponentColumn(libraryUser -> {
				Div fl = new Div();
				fl.getStyle().set("textAlign", "right");
		...
		case AUTHOR:
			...
			gridAuthors.addColumn(libraryUser -> libraryUser.getCreatedDate().format(DateTimeFormatter.ISO_DATE))
					.setHeader("Created");
			gridAuthors.addComponentColumn(author -> {
				Div fl = new Div();
				fl.getStyle().set("textAlign", "right");
		...
		case PUBLISHER:
			...
			gridPublisher.addColumn(publisher -> publisher.getCreatedDate().format(DateTimeFormatter.ISO_DATE))
					.setHeader("Created");
			gridPublisher.addComponentColumn(publisher -> {
				Div fl = new Div();
				fl.getStyle().set("textAlign", "right");
	...
	private void deleteLibraryUserView(LibraryUser libraryUser, EntityAction entityAction) {
		...
		new AddEntityDiv(libraryUser, BeanAction.DELETE, entityAction, i -> {
			i.delete();
	...
	private void editLibraryUserView(LibraryUser libraryUser, EntityAction entityAction) {
		...
		new AddEntityDiv(libraryUser, BeanAction.EDIT, entityAction, i -> {
			i.persist(i);
			refreshDataTable(libraryUser);
		}).open();
		// refreshDataTable(libraryUser);
	...
	private void deleteAudioVideoView(AudioVideo audioVideo, EntityAction entityAction) {
		...
		new AddEntityDiv(audioVideo, BeanAction.DELETE, entityAction, i -> {
			i.delete();
	...
	private void editAudioVideoView(AudioVideo audioVideo, EntityAction entityAction) {
		...
		new AddEntityDiv(audioVideo, BeanAction.EDIT, entityAction, i -> {
			i.persist(i);
			refreshDataTable(audioVideo);
		}).open();
		// refreshDataTable(audioVideo);
	...
	private void editAuthorView(Author author, EntityAction entityAction) {
		...
		new AddEntityDiv(author, BeanAction.EDIT, entityAction, i -> {
			i.persist(i);
	...
	private void editPublisherView(Publisher publisher, EntityAction entityAction) {
		...
		new AddEntityDiv(publisher, BeanAction.EDIT, entityAction, i -> {
			i.persist(i);
	...
	private void deletePublisherView(Publication publication, EntityAction entityAction) {
		...
		new AddEntityDiv(publication, BeanAction.DELETE, entityAction, i -> {
			i.delete();
	...
	private void deleteAuthorView(Author author, EntityAction entityAction) {
		...
		new AddEntityDiv(author, BeanAction.DELETE, entityAction, i -> {
			i.delete();
			refreshDataTable(author);
		}).open();
	}
	...
	private void deletePublisherView(Publisher publisher, EntityAction entityAction) {
		log.info("deletePublisherView>" + entityAction);
		new AddEntityDiv(publisher, BeanAction.DELETE, entityAction, i -> {
			i.delete();
			refreshDataTable(publisher);
		}).open();
	}
	...
	private void editPublisherView(Publication publication, EntityAction entityAction) {
		log.info("editPublisherView>" + entityAction);
		new AddEntityDiv(publication, BeanAction.EDIT, entityAction, i -> {
			i.persist(i);
			refreshDataTable(publication);
		}).open();
	}
	...
	private <E extends BaseEntity> void addEntityCRUD(Object obj, EntityAction entityAction) {
		...
		if (obj instanceof AudioVideo) {
			new AudioVideoDialog((AudioVideo) obj, BeanAction.NEW, i -> {
				i.persist(i);
				refreshDataTable((AudioVideo) obj);
			}).open();
		}
		if (obj instanceof LibraryUser) {
			new LibraryUserDialog((LibraryUser) obj, BeanAction.NEW, i -> {
				i.persist(i);
				refreshDataTable((LibraryUser) obj);
			}).open();
		}
		if (obj instanceof Author) {
			new AuthorDialog((Author) obj, BeanAction.NEW, i -> {
				i.persist(i);
				refreshDataTable((Author) obj);
			}).open();
		}
//		if (obj instanceof Publisher) {
//			new EditPublisherDialog((Publisher) obj, BeanAction.NEW, i -> {
//				i.persist(i);
...
//        }).open();
	...
	public enum EntityAction {
		...
		PUBLICATIONS("Publication", "Publications"), AUDIOVIDEO("AudioVideo", "AudioVideos"),
		LIBRARY_USERS("Library User", "Library Users"), RENTALS("Rental", "Rentals"),
		SUBSCRIPTIONS("Subscription", "Subscriptions"), AUTHOR("Author", "Authors"),
	...
	/*
	 * private void openCustomerView(Employee employee) { new
	 * EditEmployeeDialog(employee, BeanAction.VIEW, i ->
	 * refreshDataTable()).open(); }
	 * 
	 * private void editEmployeeView(Employee employee) { new
	 * EditEmployeeDialog(employee, BeanAction.EDIT, i -> { i.persist(i);
	 * refreshDataTable(); }).open(); }
	 * 
	 * private void deleteEmployeeView(Employee employee) { new
	 * EditEmployeeDialog(employee, BeanAction.DELETE, i -> { i.delete(i);
	 * refreshDataTable(); }).open(); }
	 * 
	 * private void newPublicationView() { new EditPublicationDialog(new
	 * Publication(), BeanAction.NEW, i -> { i.save(i); refreshDataTable();
	 * }).open(); } private void updateEmployeeCount(int count) {
